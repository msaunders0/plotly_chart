...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var ylabels = otuIds.slice(0, 10).reverse().map(entry => `OTU ${entry.toString()}`);
    var yhoverLabels = otuLabels.slice(0, 10).reverse();  
...
//Bubble chart
    ...
    var newSample = data.metadata;
    var sampleArray = newSample.filter(sampleObj => sampleObj.id == sample);   
